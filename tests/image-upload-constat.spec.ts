...
test.describe("Image upload — constat flow", () => {
  ...
  test("should upload images in all constat locations and generate PDF", async ({ page }) => {
    ...
    await expect(dialog.locator('img[src^="blob:"]')).toHaveCount(1, { timeout: 15_000 });

    // Upload image 2
    fcPromise = page.waitForEvent("filechooser");
    await dialog.getByRole("button", { name: "Ajouter photo" }).click();
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(dialog.locator('img[src^="blob:"]')).toHaveCount(2, { timeout: 15_000 });

    // Verify the uploaded image has actual pixel content (not blank)
    const imgHasContent = await dialog.locator('img[src^="blob:"]').first().evaluate((img: HTMLImageElement) => {
    ...
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(drawer.locator('img[src^="blob:"]')).toHaveCount(1, { timeout: 15_000 });
    ...
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(planSituationSection.locator('img[src^="blob:"]')).toHaveCount(1, { timeout: 15_000 });
    ...
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(planEdificeSection.locator('img[src^="blob:"]')).toHaveCount(1, { timeout: 15_000 });
    ...
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(vuesGeneralesSection.locator('img[src^="blob:"]')).toHaveCount(1, { timeout: 15_000 });

    fcPromise = page.waitForEvent("filechooser");
    await vuesGeneralesSection.getByRole("button", { name: "Ajouter photo" }).click();
    await (await fcPromise).setFiles(TEST_IMAGE_PATH);
    await expect(vuesGeneralesSection.locator('img[src^="blob:"]')).toHaveCount(2, { timeout: 15_000 });
